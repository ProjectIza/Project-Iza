...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelector('#btn-verify').addEventListener('click', () => {
        showCompletionMessage(); // Verifique se o jogo foi concluído
    });
    ...
    const backgrounds = ['iza.png', 'eu.png'];
    let currentBackgroundIndex = 0;
    
    function toggleBackground(cell) {
        // Remova o fundo atual
        cell.style.backgroundImage = 'none';
    
        // Alterne para o próximo fundo na lista
        currentBackgroundIndex = (currentBackgroundIndex + 1) % backgrounds.length;
        cell.style.backgroundImage = `url(${backgrounds[currentBackgroundIndex]})`;
    
        // Você também pode adicionar ou remover classes aqui conforme necessário
        if (currentBackgroundIndex === 0) {
            cell.classList.add('select-garota');
            cell.classList.remove('select-kaue');
        } else {
            cell.classList.add('select-kaue');
            cell.classList.remove('select-garota');
        }
    }
...
function isGameCompleted() {
    ...
    for (let cell of cells) {
        if (cell.innerHTML === '' || cell.classList.contains('error')) {
            return false;
        }
    }
    return true;
